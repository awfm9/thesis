...
In general, the collapse of one security criterion will open up the door to a number of attacks which can be executed, regardless of the technique used to exploit the vulnerability. This means that the exploit of a vulnerability is the first component in any risk, while the execution of the attack is the second step. Often, the stability of one criterion is contingent on the stability of another.\\

For instance, a narrow impact on immutability of specific transactions in the consensus state allows the execution of double spend attacks, but doesn't affect the rest of the blockchain network. In contrast, a wide impact on immutability allows the reversal of any transaction history and enables retroactive censorship of transactions, thus disrupting openness as well.\\

Similarly, a narrow impact on openness can simply mean that one network participant can not execute any transactions. In some cases, however, depriving a network participant of access to the current consensus state also allows the presentation a manipulated network view, which can effectively disrupt immutability from his subjective perspective.\\

It can therefore be said that the mere exploitation of a vulnerability can already lead to the collapse of the security criteria of the consensus state, thus making the blockchain network useless for its intended purposes, even if an attack isn't necessarily executed. In general, this means that controls need to mitigate vulnerabilities or attack methods, rather than just dealing with the attacks themselves.\\
